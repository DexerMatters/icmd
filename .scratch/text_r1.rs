    ...
    for row_index in row_start..row_end {
        ...
        for cell in cells {
            ...
            for visible_column in column.max(col_start)..end.min(col_end) {
                ...
                // Only the cell's leading column carries the glyph, and only
                // when that column is itself visible. A wide cell whose leading
                // column is off-screen contributes a blank for each visible
                // continuation column, so later cells keep their columns. A
                // continuation of a fully visible glyph is left for
                // `Image::from_rows` to derive from the glyph's width.
                grid[at] = if column < col_start {
                    Slot::Glyph(blank(base))
                } else if visible_column == column {
